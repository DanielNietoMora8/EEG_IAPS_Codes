...
#     print(L)
    ...
    aux3 = np.trace(np.asmatrix((L*L.T)))
    for i in range(P):
        Kcc[i,:,:] = np.matmul(np.matmul(U,np.squeeze(K[i,:,:])),U)
        aux1 = np.trace(np.asmatrix(np.squeeze(Kcc[i,:,:])*L))
        aux2 = np.trace(np.asmatrix(np.squeeze(Kcc[i,:,:])*np.squeeze(Kcc[i,:,:])))
 ...
 #Compunting fobenious products between pairs of kernels
    ...
    for i in range(P):
        for j in range(i+1,P):
            aux = np.trace(np.asmatrix(np.squeeze(K[i,:,:])*np.squeeze(K[j,:,:])))
            aux = aux / np.sqrt(np.trace(np.asmatrix(np.squeeze(K[i,:,:])*np.squeeze(K[i,:,:]))) * np.trace(np.asmatrix(np.squeeze(K[j,:,:])*np.squeeze(K[j,:,:]))))
            M[i,j] = aux
            M[j,i] = aux

    # Solving the analytic equation
    eta = sp.linalg.solve(M,a)
